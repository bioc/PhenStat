...
vectorOutput <- function(phenTestResult, phenotypeThreshold = 0.01)
{
  ...
  if (method(phenTestResult) %in% c("MM", "TF")) {
    ...
    for (i in 1:length(Genotype_levels)) {
      ...
      for (j in 1:length(Sex_levels)) {
        ...
        DSsize = paste(DSsize, paste(
          '"',
          paste(Genotype_levels[i], Sex_levels[j], sep = "_"),
          '"',
          sep = ""
        ), sep = "")
    ...
    if (method(phenTestResult) == "RR") {
      addInfo = paste('"',
                      rownames(parameters(phenTestResult))[1],
      ...
      addInfo = paste(
        addInfo,
        '"',
        rownames(parameters(phenTestResult))[2],
      ...
      if (noSexes(phenTestResult) == 2) {
        addInfo = paste(
          addInfo,
          '"',
          rownames(parameters(phenTestResult))[3],
        ...
        addInfo = paste(
          addInfo,
          '"',
          rownames(parameters(phenTestResult))[4],
