        ...
        if (!(depVariable %in% colnames(x)))
            ...
            if(is.numeric(columnOfInterest)){
                ...
                if ((variability<0.005) && method=="MM"){ 
                    stop_message <- paste(stop_message,"Error:\nInsufficient variability in the dependent variable '",depVariable,
            ...
            if (method=="MM"){
                ...
                numberOfGroupsPassed <- length(Genotype_levels) + length(Gender_levels)
                if (numberOfGroupsPassed == 4)
                    numberOfGroupsPassed <- 3
                numberOfGroupsChecked <- 0
                ...
                for (i in 1:length(Genotype_levels)){
                    ...
                    for (j in 1:length(Gender_levels)){           
                        ...
                        if (length(unique(columnOfInterest))>=dataPointsThreshold){
                            numberOfGroupsChecked <- numberOfGroupsChecked+1
                            #message(numberOfGroupsChecked)
                        }
                    }                
                }
                if (numberOfGroupsChecked < numberOfGroupsPassed)
                    passed <- FALSE
                if (!passed)
                stop_message <- paste(stop_message,"Error:\nInsufficient variability in the dependent variable '",depVariable,
        ...
        if (equation=="withWeight" && method=="MM"){
            ...
            if (length(columnOfInterest) == length(columnOfWeight))
                if (sum(columnOfInterest-x$Weight) == 0){    
                 if (outputMessages)
                  message("Warning:\nWeight and dependent variable values seemed to be equivalent. Equation 'withWeight' can't be used in such a case and has been replaced to 'withoutWeight'.\n")
                 equation <- "withoutWeight"
            }
